...
Vector3D Material::shadeSpecular(Vector3D ks, Vector3D viewVector, Vector3D lightColor, Vector3D lightVector, Vector3D normal) {
	...
	Vector3D temp = ks.multiply(lightColor).scale(specularComponent);
	return temp;
}
